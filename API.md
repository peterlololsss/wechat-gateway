...
| 变量名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `VPS_WEBHOOK_URL` | string | `https://<OPENCLAW_WEBHOOK_HOST>/webhook/ntchat` | Webhook 推送目标地址 |
...
**引用回复消息的特殊处理**:
对于引用回复 (quoted reply) 类型的消息 (`wx_sub_type == 57`)，Bridge 会把当前这条回复消息的文本（外层 `<title>`）作为 `content`。被引用的原消息会解析到 `quoted_message` 字段中（见 `quoted_message 字段` 一节）。如果被引用消息的实际发送者指向 Bot 自身，Bot 的 wxid 会被自动加入 `at_user_list`；群聊引用回复会优先使用 `<refermsg><chatusr>`，否则回退到 `<refermsg><fromusr>`。
